...
是否新写 README 由用户决定: 完成模板时若尚未决定可简短询问, 未答复则不新建, 不阻塞代码与验证收尾。
已有或用户选定的入册 README 面向赛场速查: 只放选择条件、性质公式、接口用法、修改位置与易错边界; 保留必要的容量、数值和栈限制。不混入学习状态/路线、对拍与 CI 记录、维护/迁移历史或站外学习资源, 这些归各自文档。公式用手册支持的数学写法并检查实际 PDF, 不以行内代码代替所有公式。
已有授权在同一范围持续有效; 现有说明随契约修正。用户选择编写时, 写适用信号、不变量/推导与易错边界。
