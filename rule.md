...
用法入口与限制; 有影响的正文同轮更新, 无影响的保持原样, 无须逐项报告未改动文件。
...
交付与记录尺度统一执行 [以最终结果为准](rules/collab.md#以最终结果为准)，不因上述同步要求新增逐步 diff、重复实施报告或强制历史条目。
...
- 这份文件维护现行契约，优先合并或替换已有条款，不追加逐轮变更史；既有历史按需查阅，无须为日常修改另存原文快照。
  常备 400 行以内。
  文件太长, AI 会开始静默忽略边角条款。
...
家族注记: 替罪羊 α=0.75 真删除+原位重建; LCA 主力 = DFN_LCA;
  ...
  优化建图以 GraphBuilder<W> 统一 g/tot/new_node, 构造第一参数为整图点容量, 第二参数只预留边; init(n) 清图保留初始状态点, 预算固定。
  prefix_in/out、suffix_in/out 返回全局入口数组, SegLinks/TreeLinks 接同一上下文和 1-based 图点映射, build 只追加骨架并重建自身索引, 不清共享图。
  汇集/分发方向独立, SegLinks/TreeLinks 可用不同源/目标映射; 结构和覆盖只在所属上下文未清空且仍存活时有效, 状态取反留在应用层。
  GraphBuilder.add 忽略 0 入口; link 按全体元素遍历覆盖, 空集合不改图, 任一侧单入口直接连边, 双侧多入口用一中继且入零出 w, 可跨任意压缩结构。
  SegLinks 区间覆盖 O(log n), TreeLinks 复用外置静态 dep/fa/lca 每路径至多4个可重叠段、定位 O(T+log n), 原树边不自动加入, 断连覆盖全0。
  旧 PrefixGraph/SuffixGraph/PrefixSuffixGraph、SegGraph、TreeGraph 共用新内核, 原构造与连边及 build 清图语义保留, 第三参数仍为手动中继预算; 带内部引用的旧封装禁止拷贝和移动。
  前后缀旧封装单向骨架 3n-2 点/4n-4 边, 混合 5n-4 点/8n-8 边; SegGraph 骨架3n-2点, r2r/r2new/p2new 各耗一中继。
  graphbuilder_check 默认验证三种边权1200组共享映射组合、1000组P6378穷举与20万变量百万操作, 原三份对拍继续覆盖兼容接口和原有规模。
  取舍、容量与迁移见 [家族组合设计](docs/features/design/图论.md#优化建图家族的组合设计); 不保持路径条数或网络流容量, 建图通过不代表下游递归 SCC 的百万规模栈/内存已验。
