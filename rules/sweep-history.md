...
本页保留既有记录供追溯，不再要求每件任务完成后追加。日常交付与必要记录执行 [协作约定](collab.md#以最终结果为准)；当前状态以现行文档、验证表和工作板为准。

## 2026-09-15 Astra: 外部 P_3513.cpp 只读审阅

- 定位自环误约束、跨组度数只计一端、遗漏单人移动、初始空组误计四处问题；末次读取确认用户已自行补上跳过 i == j，助手未修改外部源码或模板。
- 完整修正版与验证器见 `.zoi-checks/codex-work/p3513-review-20260915/`。Windows MinGW GCC、C++20、O2，使用本库 zoi 的 Graph/SCC；枚举 n=2..6 全部 33866 张无向图，与枚举所有非空真子集的独立暴力一致。原始样例输出 0，只修自环和度数输出 1，完整修正输出 3。
- 修正版使用单字节邻接矩阵、按实际边数预留原图、取消未使用缩点图的边预留。未做 n=5000 压测或在线提交，不作完整性能验收；未更新用户学习状态，未提交或推送。

## 2026-09-11 Astra: Windows PowerShell 手册审计修复

- 复现 PS 5.1 将 Typst 表达式中的引号丢失，导致 PDF 编译后审计失败；生产查询改用 ProcessStartInfo 的参数列表/CRT 转义、显式 UTF-8 解码和真实退出码。
- PS 5.1/7 各通过 18 次 Render，最终 AuditOnly 双版本通过；Windows CI 补真实审计调用，原有发现/渲染检查保留。正式手册已用用户原终端重建为 143 页，106 条目、189 目录、4 说明、32 公式及第 39 页页脚均通过检查，目录和页脚另做视觉抽查。
- 同步规则、功能与使用/维护文档，证据见[专项记录](../records/tooling/booklet-argv-20260911.md)。本轮临时文件存证后清理；原学习记录、UVA1391 历史和用户 `_snap.ps1` 保留。未提交、未推送，线上 CI 尚未运行。

## 2026-09-10 Codex: UVA1391 例题检查

- 只检查并测试库外 `F:/Microsoft VS Code/code_set/oj_test/UVA_1391.cpp` 的副本, 原文件不改。两处笔误: 主循环将 n/m 读反, 选择年龄限定任务时 A/B 输出反了; 建图与本库 SCC 小编号取真的方向正确。
- Windows / g++ 15.2 / C++20 O2、标准库断言: 仅修输入后, `1 0 / 20 / 0 0` 输出不合法的 B; 再修 A/B 后, 固定种子 1391 的 600 组 n<=8 多测通过独立二选一穷举, 同时检查无解判断、年龄限制和所有冲突边。此为小规模建模验证, 不宣称 OJ 或目标规模通过。
- 记录结果后清理本轮 `uva1391-20260910` 临时目录; 2-SAT 学习状态沿用正在学, 未提交、未推送。

## 2026-09-09 Codex: 全库生命周期接口统一

- 按 catalog 逐项核对 78 份现役源码, 74 份审查、4 份欧拉按用户指令暂缓; 19 份引擎实现/接口调整, 优化建图与虚树的 4 份 Usage 同步。build/run 自带必要复位, 显式 n 与容量分离; 保留输入和持久化版本, HLD 旧二参数接口拒绝编译以免误解 root。
- 新增生命周期和线段树套 FHQ 两套默认回归, 更新原相关测试、登记和两层表。Windows 全量 135/135 及最终受影响套件补跑通过; Linux 15 套 ASan/UBSan 通过; 百万点 LCA/虚树、20 万点图论压力通过, 两个默认栈递归探针仍为明确 STACK_LIMIT。
- 逐项清单、迁移表、真实 JSON 与未覆盖边界见[生命周期记录](../records/verification/lifecycle-20260909.md)。同步 rule 与用户功能说明; 本人学习状态不提升。文档和生成表检查通过后已清理本轮 834 个临时文件, 用户 scripts/_snap.ps1 与库外题解不动; 未提交、未推送。

## 2026-09-09 Codex: 图论容量与小测复用检查

- 用户要求追查拓扑同类问题；33 份现役图论代码中按新指令排除欧拉四份，最终检查 29 份。统一 Graph 的 25 份未发现拓扑以外的整容量复制；旧 Floyd 整矩阵 memset 改为只清本轮方阵，保留原 standalone main。
- 新增两份自动 CI 容量回归：25 份 Graph 体系的 20 万预留、四组各 1000 轮小测及零边预留扩容；Floyd 304 组独立 Dijkstra 与矩阵范围哨兵。旧 Floyd 先触发新增断言；修复后 Windows 和 Linux ASan/UBSan 均通过，证据见[专项报告](../records/verification/graph-capacity-20260909.md)。
- 同步验证登记、生成目录与状态表、规则和功能说明；欧拉四份记录本人待复习与本轮排除，其源码未动。说明一次构造/上轮清理/显式 Graph 拷贝的必要成本；临时产物存证后清理，未提交或推送。

## 2026-09-08 Codex: 拓扑排序多测复制修复入库

- 用户授权后将拓扑母版改为仅复制本轮 0..n 入度，新增 8520710 预留容量下 1000 组小测回归，旧版先失败、修复后通过；同步契约、Usage、rule、功能表、验证登记和两层生成表。
- Windows 四套件通过；Linux ASan/UBSan 两个拓扑相关完整套件通过，含原有 20 万实际点数验证。证据与限制见[专项报告](../records/verification/topo-20260908.md)。未提交、未推送、未宣称 OJ 或远程 CI 已通过。
- 外部 P9520 由用户自行改过，不覆盖；学习状态无需更新。测试产物存证后清理，保留先前排障记录。

## 2026-09-08 Codex: P9520 多测拓扑复制耗时定位

- 只测试用户展开源码的副本，未修改例题或库模板。TG 原点容量 240020、中继容量 120010，使最终图容量为 8520710 点；TopoSort 的整份入度复制每轮处理 34082880 字节，与本轮真实点数无关。
- WSL Ubuntu-20.04 / g++-13 / C++20 O2、256 MiB 栈：1000 组 n=120 的链、每组囚犯 2->3 和 3->4、总 n=120000，原版 2.367 秒，副本仅将 `in = g.in_deg` 改为 `in.assign(g.in_deg.begin(), g.in_deg.begin() + n + 1)` 后 0.263 秒，全部输出 Yes。
- 单组 n=m=120000 链、每个 i 移动到 i%n+1：前后均输出 No，分别 0.737 / 0.884 秒；该改动针对多测整容量复制，不宣称单组提速。未取得 OJ 失败输入或重提交结果，不将本地定位等同于 AC。
- 结果记录后按精确目录清理本次临时脚本、两个源码副本和可执行文件。现行模板尚未改变，正式模板的验证状态不变。

## 2026-09-08 Codex: P9520 例题 RE 定位

- 外部例题 `F:/Microsoft VS Code/code_set/oj_test/P_9520.cpp` 由用户同时编辑，本次仅测试副本，未覆盖源码、展开状态或修改库模板。
- WSL Ubuntu-20.04 / g++-13 / C++20 O2、标准库断言：旧 LCA 容量 1000 在 n=506、建表点数 1012 时断言退出 -6；仅扩大 LCA 后，TG 原点容量 10000 在 n=5001、建表点数 10002 时断言退出 -6。TG 构造参数依次为原点容量、预留边数、中继预算。
- 副本容量改为 G/LCA/TG 原点 240020、TG 中继 120010，TG 预留 16000000 边：CPH 两组输入（三个样例）全部符合输出；12 万点星形树通过；12 万点链在 8192 KiB 栈退出 -11，262144 KiB 栈输出 Yes。两种大树使用 2->3、3->4 两个囚犯，预期 Yes；仅用于容量和栈复现，不代表完整例题对拍或 OJ 验收。
- 收尾观察用户已将三项点容量自行改为 240020、TG 中继改为 120010；递归 LCA 的深链栈限制仍存在。临时脚本、源码副本和可执行文件在记录结果后按精确目录清理；学习状态及模板验证表无需改动。

## 2026-09-08 Codex: 图论专项 CI 入口修复

- CI #38 的两个专项作业在编译前失败：suite 局部变量遮蔽定位函数、依赖扫描误读 Usage 内的 include；修复脚本并扩充既有自检到两个真实 profile 启动路径及注释/真实依赖区别
- WSL 自检 4 项通过；LCA/虚树百万点与综合图论 20 万点两种完整 profile 跑通，原默认栈探针按既定契约报告 STACK_LIMIT；没有降低规模或放宽失败条件
- 证据与范围见[修复记录](../records/verification/ci-stress-20260908.md)，同步规则与功能事实；算法、学习状态和默认验证登记无需改动，线上结果按修复提交的 Actions 核对

## 2026-09-08 Codex: 目录自动同步与数论归档

- 保留用户质数筛目录与 PIL 修改；约数公式归入约数目录，整数开方/除法取整并入 utils，撤下两份独立跳板；原学习确认与两个知识点保留
- 新增纯 PowerShell 路径同步，以源码稳定短名更新 catalog/跳板/include/已有文档链接与验证路径；展开、对拍、手册、跳板、打包与 CI 接入；专项脚本按短名/唯一测试名定位。合理人工登记及歧义边界写入维护指南
- Windows 32 个对拍及 99 个语法项通过，Linux sanitizer 32/32；最终重复 include 用例另补验；同步器 PS5.1/PS7 各 11 项通过，六类既有工具双版本最终通过；utils Usage、BigInt/rw 深测及文档/设计检查通过
- 证据与已知限制见[实施记录](../records/tooling/layout-20260908.md)，任务产物按清单哈希清理；未渲染正式 PDF、未提交或推送
