...
public:
    ...
    {
        u128 limit = neg ? (u128(1) << 127) : u128(numeric_limits<i128>::max());
        u128 r = 0;
        for (size_t i = a.size(); i-- > 0; )
        {
            assert(r <= (limit - a[i]) / B && "to_i128(): 超出可表示范围");
            r = r * B + a[i];
        }
        return neg && r ? -i128(r - 1) - 1 : i128(r);
    }
...
int main()
{
    // 输入两个合法十进制整数, b 非零; 可带一个正负号和前导零
    BigInt a, b;
    if (!(cin >> a >> b)) return 0;
    cout << a + b << '\n';
    cout << a - b << '\n';
    cout << a * b << '\n';
    auto [q, r] = a.divmod(b);             // 同时要商余时只做一次除法
    cout << q << ' ' << r << '\n';        // 商向零截断, 非零余数与 a 同号
    cout << a.mod(97) << '\n';            // 归一化到 [0, 97), 不同于 a % 97
    cout << a.abs().sqrt() << '\n';       // 非负整数的下取整平方根
    cout << gcd(a, b) << ' ' << lcm(a, b) << '\n'; // 均非负, gcd(0,0)=0

    BigInt x = "-000123";                 // 规范化为 -123; "-0" 规范化为 0
    x += 200; x -= 7; x *= 3; x /= 2; x %= 11;
    cout << x << ' ' << 100 - x << '\n';  // 6 94, 原生整数可放两侧
    cout << (x == 6) << ' ' << (x >= 0) << '\n';
    cout << x.sign() << ' ' << x.digits10() << '\n'; // 1 1, 位数不含符号
    cout << x.is_zero() << ' ' << bool(x) << '\n';  // 0 1
    string s = x.str();                   // 也可 to_string(x)
    x = s;
    cout << x.to_LL() << '\n';            // 只在结果落入 LL 范围时转换

    BigInt p = BigInt(2).pow(100);         // 非负指数; 0^0=1
