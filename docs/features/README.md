...
对拍现状两层表只列正式登记的模板, 例题、临时题解和其他豁免代码不列入

自动更新由对拍脚本驱动, 不依赖 AI 再手动填表. 人或 AI 负责核实并登记 API、暴力和边界范围, 脚本负责记录真实运行结果并生成两层表; 没登记的范围仍显示未知

只改代码不会触发监听刷新, 要运行 `make_verification.ps1` 才会重算待重验状态. CI 的本次表格随日志附件提供, 不自动回写仓库; 导入附件中的证据后再生成, 才会合并到库内概览
