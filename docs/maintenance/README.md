...
是否需要新增记录先按 [以最终结果为准](../../rules/collab.md#以最终结果为准) 判断；上述导航要求只适用于确有必要新增的文档，不要求每次修改都创建专项报告、迁移对照或原文快照。普通维护直接更新现行正文，收尾概括最终结果与验证即可。
...
- 不再为日常维护强制追加历史；已有记录默认保留。用户要求整理历史时，保留仍有价值的结论、验证依据和未解决分歧，合并重复过程，不扩大原有通过结论。
...
以上是通用缓存清理器的扫描边界。AI 创建者仍须在任务收尾时按 [工作区约定](../../rules/collab.md#工作区与沉淀) 删除自己已用完的一次性测试源码、脚本、输入输出、可执行文件及临时 diff，无须等待缓存清理器处理。正式回归、极限对拍、CI 依赖和最终交付物保留；未解决问题只保留最小必要复现。
...
清理前确认仍需支撑最终结论的证据已由自动报告或必要的专项记录保留；不为清理无用中间产物补写操作历史。两层验证表不依赖本地原始日志存活。
