...
// - rng() % k - c 型负值采样是无符号下溢: 本想要 [-c, k-c), 实际 rng()%k<c
//   时得无符号巨数(Windows uint_fast32_t=4B 得 4e9, Linux=8B 得 1.8e19 直接
//   溢出 LL)——必须写 (int)(rng() % k) - c 先窄化再减。CI 首日实测: 该下溢
//   让树重心的负点权路径从未被真正测过, 引擎 mx=0 哨兵残留 bug 被掩盖多
//   年, 修完测试当轮现形——本地绿掩盖真病灶的又一案。
// - 负权场景 max 哨兵禁用 0: 候选块可全为负, 哨兵取 LLONG_MIN(树重心 dfs
//   实测; 顺带: up 块必须在子树累加完成后算, 挪到循环前是低级错)。
// - 跨平台 CI 是库体检机: 首日六轮揪出五个本地永远抓不到的病——BOM
//   (PS5.1 Set-Content -Encoding UTF8 带毒, 二维树状数组中招)/git 索引留
//   旧小写桩名(Linux 大小写敏感检出即缺件, 见 §13 大小写条款)/include
//   层数数错(欧拉×2, TEST GAP 盲区靠语法扫兜底)/全局变量 visit 撞
//   std::visit(g++13 非限定名歧义, 12/15 都容忍)/constexpr vector 比较
