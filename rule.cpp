...
//
// 最近一轮: 注释=侵入面标记 | 访问控制判据(kth_of 正例/SegTree build 反例) | 私有沉底单标签(全库 27 文件 28 结构体统一)
...
//   相对引用 ../。总入口 scripts\run_checks.ps1。现役 12 套:
...
//   oset_check(平衡树六件+FHQ_Seq 双域) | trie_check | mint_check |
